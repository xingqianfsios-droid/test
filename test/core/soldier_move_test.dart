...
void main() {
  group('兵/卒 (Soldier) 走法测试', () {
    ...
    test('红兵过河后可前进和左右', () {
      // row <= 4 为过河
      final pieces = [
        const PieceModel(type: PieceType.soldier, side: PieceSide.red, col: 4, row: 4),
      ];
      final board = buildBoard(pieces);
    ...
    test('黑卒过河后可前进和左右', () {
      // row >= 5 为过河
      final pieces = [
        const PieceModel(type: PieceType.soldier, side: PieceSide.black, col: 4, row: 5),
      ];
      final board = buildBoard(pieces);
    ...
    test('红兵在左边界过河后只能前进和右走', () {
      final pieces = [
        const PieceModel(type: PieceType.soldier, side: PieceSide.red, col: 0, row: 3),
      ];
      final board = buildBoard(pieces);
    ...
    test('红兵在最顶行只能左右走', () {
      final pieces = [
        const PieceModel(type: PieceType.soldier, side: PieceSide.red, col: 4, row: 0),
      ];
      final board = buildBoard(pieces);
    ...
    test('兵不能吃前方己方棋子', () {
      final pieces = [
        const PieceModel(type: PieceType.soldier, side: PieceSide.red, col: 4, row: 6),
        const PieceModel(type: PieceType.soldier, side: PieceSide.red, col: 4, row: 5),
      ];
      final board = buildBoard(pieces);
      final moves = getSoldierMoves(col: 4, row: 6, side: PieceSide.red, board: board);
